...
router.post('/display_all_vehicle_searching', function (req, res, next) {
    pool.query("select V.*,(select C.categoryname from category C where C.categoryid=V.categoryid) as categoryname, (select S.subcategoryname from subcategory S where S.subcategoryid=V.subcategoryid) as subcategoryname,(select CM.companyname from company CM where CM.companyid=V.companyid) as companyname,(select M.modelname from model M where M.modelid=V.modelid) as modelname  from vehicle V where V.companyid=? in(select C.companyid from company C where C.compnayid in (?))", [req.body.companyid], function (error, result) {
...
router.post('/check_user_mobileno', function (req, res, next) {
    pool.query("select * from userdetails where mobileno=?",
        [req.body.mobileno], function (error, result) {
            if (error) {
                res.status(500).json({ status: false, message: 'Server Error' })
            }
            else {
...
router.post('/userdetailssubmit', function (req, res, next) {
    pool.query
        ('insert into userdetails(mobileno,emailid,fullname,birthdate,aadharno,licenseno)values(?,?,?,?,?,?)', [req.body.mobileno, req.body.emailid, req.body.fullname, req.body.birthdate, req.body.aadharno, req.body.licenseno], function (error, result) {
            if (error) {

...
router.post('/vehiclesubmit', upload.any(), function (req, res, next) {
    ...
    pool.query("insert into vehicle(categoryid,subcategoryid,companyid,modelid,vendorid,registrationno,color,fueltype,ratings,average,remarks,capacity,status,feature,icon) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        [req.body.categoryid,
...
router.post('/edit_picture', upload.single('icon'), function (req, res, next) {
    ...
    pool.query("update vehicle set icon=? where vehicleid=?", [req.file.filename, req.body.vehicleid], function (error, result) {
        if (error) {
            ...
            fs.unlinkSync(`${"E:/paynrent_backend/public/images"}/${req.body.oldicon}`)
...
router.post('/edit_data', upload.single('icon'), function (req, res, next) {
    console.log(req.file)
    pool.query("update vehicle set categoryid=?,subcategoryid=?,companyid=?,modelid=?,vendorid=?,registratinno=?,color=?,fualtype=?,ratings=?,average=?,remarks=?,capacity=?,status=?,feature=? where vehicleid=?", [req.body.categoryid,
    req.body.subcategoryid,
    req.body.companyid,
    req.body.modelid,
    req.body.vendorid,

...
export default function UserSignUpDrawer(props) {
 ...
 const GenerateOtp=()=>{
  ...
  const list = (anchor) => (
    ...
    <Grid item xs={12}>
     <TextField onChange={(event)=>setMobile(event.target.value)} variant='outlined' fullWidth label={<span style={{width:24,fontFamily:'Poppins',fontWeight:700}}>Mobile Number</span>}></TextField>
    </Grid>
    <Grid item xs={12}>
    ...
    <Grid item xs={12}>
     ...
     <OtpInterface  handleClose={handleClose} getOtp={getOtp} generatedOtp={generatedOtp} GenerateOtp={GenerateOtp} onChange={handleOtpChange} mobile={mobile}/>
    </Grid>:<></>}
        </Grid>
    );

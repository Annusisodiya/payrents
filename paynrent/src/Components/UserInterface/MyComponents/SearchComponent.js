...
export default function SearchComponent(props) {
  ...
  const fetchAllCities = async () => {
    var response = await getData('userinterface/display_all_cities')
...
const handleCityDialog = () => {
  ...
  const handleClick=()=>{
   if((days>=0 && hrs>0) ||(days>0 && hrs>=0))  {
    dispatch({type:'ADD_BOOKING',payload:{city:selectedCity,startTime:startTime,endTime:endTime,direction:daysTime,days:days,hrs:hrs}})
    ...
    <div style={{ position: 'relative' }}>
      ...
      <div style={{ position: 'absolute', left: '5%', top: '4%' }}>
        <Paper elevation={3} style={{ display: 'flex', alignItems: 'center', padding: 10, borderRadius: 15, width: 500, height: 460, flexDirection: 'column' }}>
          <div style={{ position: 'relative', background: '#54a0ff', width: 480, height: 90, borderRadius: 15 }}>
          ...
          <div onClick={handleClick} style={{ cursor: 'pointer', background: '#54a0ff', marginTop: '5%', padding: 10, alignItems: 'center', justifyContent: 'center', display: 'flex', width: 380, height: 40, borderRadius: 20 }}>
